...
class LaserScanToPointcloud {
	...
	public:
		...
		LaserScanToPointcloud(std::shared_ptr<rclcpp::Node> node, std::string target_frame = "", double min_range_cutoff_percentage = 1.05, double max_range_cutoff_percentage = 0.95, int number_of_tf_queries_for_spherical_interpolation = 4, double tf_lookup_timeout = 0.2);
		...
		bool integrateLaserScanWithShpericalLinearInterpolation(const sensor_msgs::msg::LaserScan::SharedPtr& laser_scan);
		bool lookForTransformWithRecovery(tf2::Vector3& translation_out, tf2::Quaternion& rotation_out, const std::string& target_frame, const std::string& source_frame, const rclcpp::Time& time, const rclcpp::Duration& timeout = rclcpp::Duration::from_seconds(0.2));
		bool lookForTransformWithRecovery(tf2::Transform& point_transform_out, const std::string& target_frame, const std::string& source_frame, const rclcpp::Time& time, const rclcpp::Duration& timeout = rclcpp::Duration::from_seconds(0.2));
		bool updatePointTransformWithMotionEstimation(tf2::Transform& motion_estimation_transform_in_out, tf2::Vector3& translation_in_out, tf2::Quaternion& rotation_in_out, const std::string& motion_estimation_target_frame, const std::string& motion_estimation_source_frame, const rclcpp::Time& time, const rclcpp::Duration& timeout = rclcpp::Duration::from_seconds(0.2));
		// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   </LaserScanToPointcloud-functions>  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		...
		inline rclcpp::Duration getTfLookupTimeout() const { return tf_lookup_timeout_; }
		...
		inline void setTFLookupTimeout(double tf_lookup_timeout) { tf_lookup_timeout_ = rclcpp::Duration::from_seconds(tf_lookup_timeout); }
	...
	private:
		...
		rclcpp::Duration tf_lookup_timeout_;
