...
LaserScanToPointcloud::LaserScanToPointcloud(std::shared_ptr<rclcpp::Node> node, std::string target_frame, double min_range_cutoff_percentage, double max_range_cutoff_percentage, int number_of_tf_queries_for_spherical_interpolation, double tf_lookup_timeout) :
		...
		tf_lookup_timeout_(rclcpp::Duration::from_seconds(tf_lookup_timeout)),
...
bool LaserScanToPointcloud::integrateLaserScanWithShpericalLinearInterpolation(const sensor_msgs::msg::LaserScan::SharedPtr& laser_scan) {
	// laser info
	size_t number_of_scan_points = laser_scan->ranges.size();
	size_t number_of_scan_steps = number_of_scan_points - 1;
	rclcpp::Duration scan_duration = rclcpp::Duration::from_seconds((double)number_of_scan_steps * (double)laser_scan->time_increment);
	
	// Use ROS time source (respects use_sim_time parameter)
	rclcpp::Time scan_start_time(laser_scan->header.stamp.sec, laser_scan->header.stamp.nanosec);
	rclcpp::Time scan_middle_time = scan_start_time;
	
	if (laser_scan->time_increment > 0.0) {
		int64_t middle_offset_nanos = static_cast<int64_t>(scan_duration.seconds() * 1e9 / 2.0);
		scan_middle_time = scan_start_time + rclcpp::Duration::from_nanoseconds(middle_offset_nanos);
	}

	std::string laser_frame = laser_frame_.empty() ? laser_scan->header.frame_id : laser_frame_;

	// tfs setup  
	rclcpp::Time tf_query_time = ((number_of_tf_queries_for_spherical_interpolation_ < 2) || (laser_scan->time_increment <= 0.0)) ? scan_middle_time : scan_start_time;
	...
	double laser_slice_time_increment_double = ((number_of_tf_queries_for_spherical_interpolation_ < 2) || (laser_scan->time_increment <= 0.0)) ? scan_duration.seconds() : scan_duration.seconds() / (double)(number_of_tf_queries_for_spherical_interpolation_ - 1);
	rclcpp::Duration laser_slice_time_increment = rclcpp::Duration::from_seconds(laser_slice_time_increment_double);

	rclcpp::Time past_tf_time = scan_start_time;
	...
	rclcpp::Time future_tf_time = past_tf_time + laser_slice_time_increment;
	...
	rclcpp::Duration point_time_increment = rclcpp::Duration::from_seconds(laser_scan->time_increment);
	rclcpp::Time current_point_time = scan_start_time;
	...
	for (size_t point_index = 0; point_index < number_of_scan_points; ++point_index) {
		...
		if (point_range_value > min_range_cutoff && point_range_value < max_range_cutoff) {
			...
			if (future_tf_valid) {
				current_interpolation_ratio = (current_point_time - past_tf_time).seconds() / laser_slice_time_increment_double;
		...
		if (future_tf_valid) {
			current_point_time += point_time_increment;
...
bool LaserScanToPointcloud::lookForTransformWithRecovery(tf2::Vector3& translation_out, tf2::Quaternion& rotation_out, const std::string& target_frame, const std::string& source_frame, const rclcpp::Time& time, const rclcpp::Duration& timeout) {
	...
	if (!transform_available) { // try to recover using [ sensor_frame -> recovery_frame -> target_frame ]
		if (recovery_frame_.empty()) {
			RCLCPP_WARN_STREAM(rclcpp::get_logger("laserscan_to_pointcloud"), "Laser assembler couldn't get TF [ " << source_frame << " -> " << target_frame << " ] at time " << time.seconds() << " with TF timeout of " << timeout.seconds() << " seconds");
			return false;
		}
		...
		if (!transform_available) {
			RCLCPP_WARN_STREAM(rclcpp::get_logger("laserscan_to_pointcloud"), "Laser assembler couldn't get TF [ " << source_frame << " -> " << recovery_frame_ << " ] at time " << time.seconds() << " with TF timeout of " << timeout.seconds() << " seconds");
			return false;
		}

		RCLCPP_WARN_STREAM(rclcpp::get_logger("laserscan_to_pointcloud"), "Recovering from lack of tf between " << source_frame << " and " << target_frame << " using " << recovery_frame_ << " as recovery frame");
...
bool LaserScanToPointcloud::lookForTransformWithRecovery(tf2::Transform& point_transform_out, const std::string& target_frame, const std::string& source_frame, const rclcpp::Time& time, const rclcpp::Duration& timeout) {
	if (source_frame == target_frame) {
...
bool LaserScanToPointcloud::updatePointTransformWithMotionEstimation(tf2::Transform& motion_transform_in_out, tf2::Vector3& translation_in_out, tf2::Quaternion& rotation_in_out, const std::string& motion_estimation_target_frame, const std::string& motion_estimation_source_frame, const rclcpp::Time& time, const rclcpp::Duration& timeout) {
	...
	RCLCPP_WARN_STREAM(rclcpp::get_logger("laserscan_to_pointcloud"), "Laser assembler couldn't get TF [ " << motion_estimation_source_frame << " -> " << motion_estimation_target_frame << " ] at time " << time.seconds() << " with TF timeout of " << timeout.seconds() << " seconds");
