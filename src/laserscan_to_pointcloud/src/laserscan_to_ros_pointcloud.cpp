...
LaserScanToROSPointcloud::LaserScanToROSPointcloud(std::shared_ptr<rclcpp::Node> node, std::string target_frame, bool include_laser_intensity, double min_range_cutoff_percentage, double max_range_cutoff_percentage) :
		LaserScanToPointcloud(node, target_frame, min_range_cutoff_percentage, max_range_cutoff_percentage),
...
void LaserScanToROSPointcloud::initNewPointCloud(size_t number_of_reserved_points) {
	pointcloud_ = std::make_shared<sensor_msgs::msg::PointCloud2>();
	...
	if (include_laser_intensity_) {
		...
		pointcloud_->fields[3].datatype = sensor_msgs::msg::PointField::FLOAT32;
